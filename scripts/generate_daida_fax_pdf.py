...
def draw_pdf() -> str:
    ...
    text(c, LEFT + 31 * mm, 217.5 * mm, "欠勤のたびに、一人ずつ電話・調整", 12)
    ...
    fx, fy, fw, fh = LEFT, 133.0 * mm, WIDTH, 15.5 * mm
    rect(c, fx, fy, fw, fh, 1.6)
    # 「お申し込み」ではなく「ご登録」と書く。利用規約・特定商取引法に基づく表記が
    # どちらも「初回登録した店舗」を条件にしており、問い合わせではなく登録が基準のため。
    text(c, PAGE_W / 2, 142.5 * mm, "2026年10月31日までのご登録で 3か月無料", 16, "Meiryo-Bold", "center", limit=fw - 8 * mm)
    ...
    text(c, PAGE_W / 2, 136.0 * mm, "無料期間終了後に代理募集を利用するには、有料プランへの加入が必要です", 10.5, "Meiryo-Bold", "center", limit=fw - 8 * mm)

    text(c, LEFT, 127.0 * mm, "有料プラン（税込）　※自動的に有料プランへ移行することはありません", 11)
    text(c, LEFT, 121.5 * mm, "※同じメールアドレスで2店舗目以降を登録される場合、無料期間は適用されません。", 10.5)

    px, py, pw, ph = LEFT, 102.0 * mm, WIDTH, 14.7 * mm
    pcol = pw / 3
